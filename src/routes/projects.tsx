...
function ProjectsPage() {
  ...
  const categories = ["All", "Portfolio", "Website", "WebApp", "Mobile App"];
            ...
            <div className="group/preview relative rounded-[2.5rem] overflow-hidden bg-accent shadow-2xl">
              <div className="aspect-[4/5] overflow-hidden">
                <img
                  src={p.image}
                  alt={p.name}
                  className="w-full h-full object-cover transition-transform duration-700 group-hover/preview:scale-110 opacity-40 mix-blend-luminosity"
                />
              </div>
              ...
              <div className="absolute inset-0 p-8 lg:p-10 flex flex-col justify-end bg-gradient-to-t from-accent via-accent/40 to-transparent">
                ...
                <div className="flex gap-3">
                  <a href={p.repo} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 backdrop-blur-md px-6 py-4 rounded-2xl text-sm font-bold transition-all border border-white/10">
                    <Github className="w-5 h-5" /> Source Code
                  </a>
                  {p.live ? (
                    <a href={p.live} target="_blank" rel="noreferrer" className="flex-1 flex items-center justify-center gap-2 bg-primary text-primary-foreground px-6 py-4 rounded-2xl text-sm font-bold hover:shadow-[0_0_30px_rgba(255,255,255,0.2)] transition-all">
